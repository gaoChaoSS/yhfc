...
<head>
	...
	<title>${message("business.product.edit")} - Powered By JDSQ</title>
								...
								<%if (initSkuValue != null) {%>
									<div class="input-group">
										...
										<div class="input-group-btn">
											<a class="sn btn btn-default" href="${base}/admin/stock/stock_in?skuSn=<%-initSkuValue.sn%>" title="${message("business.product.stockIn")}" data-toggle="tooltip">+</a>
											<a class="sn btn btn-default" href="${base}/admin/stock/stock_out?skuSn=<%-initSkuValue.sn%>" title="${message("business.product.stockOut")}" data-toggle="tooltip">-</a>
										</div>
									</div>
								<%} else {%>
			...
			$().ready(function() {
				...
				$productForm.validate({
					...
					submitHandler: function(form) {
						...
						$(form).ajaxSubmit({
							successRedirectUrl: "${base}/admin/product/list"
						});
					}
				});
					...
					<div class="panel-body">
						...
						<div class="tab-content">
							<div id="base" class="tab-pane active">
                                <div class="form-group">
                                    <label class="col-xs-3 col-sm-2 control-label item-required">${message("供应商")}:</label>
                                    <div class="col-xs-9 col-sm-4">
                                        <select id="supplierId" name="supplierId" class="selectpicker form-control"  data-live-search="true" data-size="10" >
                                            <option value="0">无供应商</option>
										[#list suppliers as supplier]
                                            <option value="${supplier.id}" title="${supplier.supplierName}" [#if product.supplier==supplier] selected[/#if]>${supplier.supplierName}--(${supplier.position})</option>
										[/#list]
                                        </select>
                                    </div>
                                </div>
								...
								<div class="form-group">
									<label class="col-xs-3 col-sm-2 control-label item-required">${message("Coupon.store.use")}:</label>
									<div class="col-xs-9 col-sm-4">
									<select name="storeIds" class="selectpicker form-control"  data-live-search="true" data-size="10" multiple>
										<option value="0" [#if product.isAllStore] selected[/#if]>所有店铺</option>
										[#list stores as store]
											<option value="${store.id}" title="${store.name}" [#if product.isAllStore==false && product.stores?seq_contains(store)] selected[/#if]>
											${store.area.fullName}(${store.name})</option>
										[/#list]
									</select>
									</div>
								</div>
								...
								<div class="form-group">
									<label class="col-xs-3 col-sm-2 control-label" for="weight">${message("Product.maxBuy")}:</label>
									<div class="col-xs-9 col-sm-4">
										<div class="input-group">
											<input id="maxBuy" name="maxBuy" class="form-control" type="text" value="${product.maxBuy}" maxlength="9">
											<span class="input-group-addon">${message("common.maxBuy.day")}</span>
										</div>
									</div>
								</div>
								[#if product.type == "GENERAL"]
									...
									<div class="form-group">
										...
										<div class="col-xs-9 col-sm-4">
											<div class="input-group">
												...
												<div class="input-group-btn">
													<a class="btn btn-default" href="${base}/admin/stock/stock_in?skuSn=${product.defaultSku.sn}" title="${message("business.product.stockIn")}" data-toggle="tooltip">+</a>
													<a class="btn btn-default" href="${base}/admin/stock/stock_out?skuSn=${product.defaultSku.sn}" title="${message("business.product.stockOut")}" data-toggle="tooltip">-</a>
												</div>
											</div>
										</div>
									</div>
