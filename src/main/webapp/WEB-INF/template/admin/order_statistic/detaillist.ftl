...
<head>
	...
	<title>订单明细统计 - Powered By JDSQ</title>
		...
		[#escape x as x?js_string]
			...
			$().ready(function() {
				...
				$("#sback").click(function(){
					...
					window.location.href="${base}/admin/order_statistic/list?type=1&beginDate=${beginDate?string('yyyy-MM-dd')}&endDate=${endDate?string('yyyy-MM-dd')}";
...
[#--									<th>${message("common.action")}</th>--]
								...
								[#if page.content?has_content]
									<tbody>
									[#list page.content as corder]
										<tr>
											<td>${corder.name}</td>
											<td style="color: red">${corder.yxnum}</td>
											<td>${corder.wxnum}</td>
											<td style="color: red">${corder.tnums}</td>
											<td>${corder.coprice}</td>
											<td>${corder.gmv}</td>
											<td style="color: red">￥${corder.tpaids}</td>
[#--											<td>--]
[#--												--]
[#--											</td>--]
										</tr>
