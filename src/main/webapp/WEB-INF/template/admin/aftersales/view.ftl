...
<head>
	...
	<title>${message("business.aftersales.view")} - Powered By JDSQ</title>
...
var imagePath = "";
[#if aftersales.picture != ""]
imagePath = JSON.parse("${aftersales.picture}");
[/#if]

console.log(imagePath)
edit_image(imagePath)
$('.kv-file-zoom').removeAttr("disabled");
function edit_image(path){
    	var imgArray=new Array()
		for ( var i = 0; i <path.length; i++) {
            var largePath = path[i].replace("thumbnail", "large");
            imgArray.push(largePath);
        }
        ...
        $("#productImage").fileinput({
            ...
            //下面几个就是初始化预览图片的配置    
             ...
             maxFileCount:0,
             minFileCount:0,
             layoutTemplates:{actionDelete:'',actionUpload:''}
        }); 
}
			
				var $review = $("button.review");
				var $storeconfirm = $("button.storeconfirm");
				...
				$review.click(function() {
					...
					bootbox.prompt({
						...
						callback: function(result) {
							...
							//console.log($("#checkReason").val())
							if(result == "FAILED" && "" == $("#checkReason").val())
							{
								bootbox.alert("请填写被驳回原因");
								return;
							}
							
							$.ajax({
								url: "${base}/admin/aftersales/review",
								type: "POST",
								data: {
						...
						callback: function(result) {
							...
							$.ajax({
								...
								data: {
									...
									reviewRs: "false",
									checkReason:$("#checkReason").val(),
									storeConfirm: result == "STORERCONFIRM" ? "true" : "false"
								},
								dataType: "json",
								cache: false,
								success: function() {
				...
				$complete.add().click(function() {
					...
					bootbox.confirm("${message("business.aftersales.completeConfirm")}", function(result) {
						...
						$.ajax({
							url: "${base}/admin/aftersales/complete",
							type: "POST",
							data: {
								aftersalesId: $element.data("id")
							},
				...
				$transitStep.click(function() {
					...
					$.ajax({
						url: "${base}/admin/aftersales/transit_step",
						type: "GET",
						data: {
							aftersalesId: $element.data("id")
						},
							...
							<div class="row">
								<div class="col-xs-6 col-xs-offset-2">
									<div class="form-group">
										<button class="review btn btn-default" type="button" data-id="${aftersales.id}"[#if aftersales.status != "PENDING"] disabled[/#if]>审核</button>
										<!--
										<button class="storeconfirm btn btn-default" type="button" data-id="${aftersales.id}"[#if aftersales.status != "PENDING"] disabled[/#if]>审核需退货</button>
								...
								<div class="col-xs-12 col-sm-6">
									<dl class="items dl-horizontal">
										...
										<dd>
											...
											<a class="text-gray" href="${base}/admin/order/view?id=${aftersales.orderItems[0].order.id}">[${message("common.view")}]</a>
										</dd>
										<dt>${message("Aftersales.member")}:</dt>
										<dd>${aftersales.member.username}</dd>
										<dt>昵称:</dt>
										<dd>${aftersales.member.attributeValue1}</dd>
										<dt>电话:</dt>
										<dd>${aftersales.member.mobile}</dd>
										...
										<dt>发起售后${message("Aftersales.reason")}:</dt>
										<dd>${aftersales.reason}</dd>
										<dt>审核结果描述:</dt>
										<dd>
											<input id="checkReason" name="checkReason" class="form-control" type="text" [#if aftersales.status != "PENDING"] disabled[/#if] value="${aftersales.checkReason}" maxlength="25">
										</dd>
										<dt>售后问题描述:</dt>
										<dd>
										...
										<dd>
											<div id="productImage1" class="tab-pane" >
												<input name="productImage"  id="productImage" type="file" disabled>
											</div>
										</dd>
