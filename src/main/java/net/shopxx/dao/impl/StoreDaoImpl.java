...
public class StoreDaoImpl extends BaseDaoImpl<Store, Long> implements StoreDao {
	...
	@Override
	public List<Store> findList(Long areaId,String name){
		TypedQuery<Store> query;
		if(areaId != null && !"".equals(name)){
			String sql="select store from Store store where store.area_id=:area_id and store.name like :name ";
			query=entityManager.createQuery(sql,Store.class);
			query.setParameter("area_id",areaId);
			query.setParameter("name","%"+name+"%");
		}else{
			String sql="select store from Store store ";
			query = entityManager.createQuery(sql, Store.class);
		}
	...
	@Override
	public List<Store> findMeetStore(Double minlat,Double maxlat,Double minlng,Double maxlng){
        TypedQuery<Store> query;
        String sql="SELECT store FROM Store store WHERE store.longitude >= :minLng " +
                " AND store.longitude <= :maxLng AND store.latitude >= :minLat " +
                " AND store.latitude <= :maxLat";
        query=entityManager.createQuery(sql,Store.class);
