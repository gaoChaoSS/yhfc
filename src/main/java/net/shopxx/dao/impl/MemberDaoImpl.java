...
public class MemberDaoImpl extends BaseDaoImpl<Member, Long> implements MemberDao {
	...
	@Override
	public Boolean promotionCodeExist(String code) {

		String sql="select count(1) count from user_promotion p where p.promotionCode='"+code+"'";
		Query query = entityManager.createNativeQuery(sql.toString());
		query.unwrap(SQLQuery.class).setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP);
		List<Map<String,Object>> map= query.getResultList();
		int count=Integer.parseInt(map.get(0).get("count").toString());
		if(count>0){
			return true;
		}
		return false;
