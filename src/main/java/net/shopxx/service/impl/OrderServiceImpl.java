...
public class OrderServiceImpl extends BaseServiceImpl<Order, Long> implements OrderService {
	...
	@Override
	public List<Order> create(Order.Type type,Store store, final Cart cart, Receiver receiver, PaymentMethod paymentMethod, ShippingMethod shippingMethod, CouponCode couponCode, Invoice invoice, BigDecimal balance, String memo) {
		Assert.notNull(type, "[Assertion failed] - type is required; it must not be null");
		Assert.notNull(cart, "[Assertion failed] - cart is required; it must not be null");
		Assert.notNull(cart.getMember(), "[Assertion failed] - cart member is required; it must not be null");
		Assert.state(!cart.isEmpty(), "[Assertion failed] - cart must not be empty");
...
//					order.setArea(store.getArea());
//					order.setPhone(cart.getMember().getMobile());
//					order.setConsignee(cart.getMember().getUsername());
//				}
//				order.setShippingMethod(shippingMethod);
//			}
		order.setAddress(store.getAddress());
	...
	public void closeOrder() {
		List<Order> orders=orderDao.findOverTimes(Status.PENDING_PAYMENT);
		if(null != orders && 0<orders.size()){
			for(Order order:orders){
				// 订单设为关闭状态
				order.setStatus(Status.PAYMENT_CLOSE);
				// 库存回滚
				List<OrderItem> items = new ArrayList<OrderItem>();
				for(OrderItem orderItem:order.getOrderItems()){
					Sku sku=orderItem.getSku();
					if(sku != null && sku.getAllocatedStock()>0) {
						sku.setAllocatedStock(sku.getAllocatedStock() - orderItem.getQuantity());
	...
	public List<OrderDetailsByStore> findAllByDate(String begin, String end, Store store) {
		return orderDao.findAllByDate(begin,end,store);
	}

	@Override
	public void upateOrderStatus(String limit) {
		List<Order> orders= orderDao.findPayNotSend(limit);
		if(null != orders && orders.size()>0){
			for(Order order:orders){
				...
				order.setShippedQuantity(count);
				order.setOrderItems(items);
				this.update(order);
			}
		}
	...
	public Order findByCodeAndStatus(String code, Status status,Store store) {
		return orderDao.findByCodeAndStatus(code,status,store);
	}
	...
	public Integer findStoreMonthCount(Store store) {
		return orderDao.findStoreMonthCount(store);
	}

	@Override
	public Integer findStoreDayNoTakeCount(Store store) {
		return orderDao.findStoreDayNoTakeCount(store);
	}

	@Override
	public Integer findStoreDayTakeCount(Store store) {
		return orderDao.findStoreDayTakeCount(store);
	}

	@Override
	public int updateRefundAmount(String orderId,double refundMoney,int refund) {
		return orderDao.updateRefundAmount(orderId,refundMoney, refund);
	}

	@Override
	public List<Order> findByStatus(Status status, Member member) {
		return orderDao.findByStatus(status,member);
	}

	@Override
	public Integer findSkuBuyCount(Member member, Sku sku) {
		return orderDao.findSkuBuyCount(member,sku);
	}

	@Override
	public Map<String,Object> findNoTakeOrderByStore(Store store) {
		try {
			Map<String, Object> data = new HashMap<>();
			List<Map<String,Object>> msg=new ArrayList<>();
			List<OrderNoticeSms> orderNoticeSmss=new ArrayList<>();
			List<Order> orders = orderDao.findNoTakeOrderByStore(store);
			// 组装短信信息
			if (orders.size() > 0) {
				for (Order order : orders) {
					...
					content.append("【聚点好货】取货通知：尊敬的客户您好，您在商城购买的商品已到小区自提站点");
					content.append(s.getAddress());
					content.append("，请在" + b.getAttributeValue2() + "内凭："+order.getOutOrderCode()+"到店领取，");
					content.append("店长咨询电话：" + b.getMobile());
					item.put("context", URLEncoder.encode(content.toString(), "UTF-8"));
					msg.add(item);

					// 创建发送记录
					OrderNoticeSms orderNoticeSms=new OrderNoticeSms();
					orderNoticeSms.setOrder(order.getId());
					orderNoticeSms.setPhone(order.getPhone());
					orderNoticeSms.setContent(content.toString());
					orderNoticeSmsService.save(orderNoticeSms);
					orderNoticeSmss.add(orderNoticeSms);
				}

				data.put("orders",orders);
				data.put("msg",msg);
				data.put("smsRecord",orderNoticeSmss);
				return data;
			}
			return null;
		}catch (Exception e){
	...
	public List queryByMemberId(Long memberId, String hours,int page,int length) {
		return orderDao.queryByMemberId(memberId,hours,page,length);
	}
	...
	public void savePayLog(int type, String param, BigDecimal amount, int status, String orderSn, Long memberId,String transaction_id) {
		orderDao.savePayLog(type,param,amount,status,orderSn,memberId,transaction_id);
	}
}