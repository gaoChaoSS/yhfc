...
<head>
	...
	[#if productCategory??]
		[@seo type = "PRODUCT_LIST"]
			...
			[/#if]
			<title>[#if productCategory.seoTitle?has_content]${productCategory.seoTitle}[#else]${seo.resolveTitle()}[/#if][#if showPowered] - Powered By JDSQ[/#if]</title>
		[/@seo]
	[#else]
		<title>${message("shop.product.list")}[#if showPowered] - Powered By JDSQ[/#if]</title>
	[/#if]
